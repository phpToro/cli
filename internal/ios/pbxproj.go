...
// pbxUUID generates a deterministic 24-char hex UUID for pbxproj entries.
func pbxUUID(parts ...string) string {
	h := md5.Sum([]byte("phptoro:" + strings.Join(parts, ":")))
	return strings.ToUpper(hex.EncodeToString(h[:12]))
}

type pbxFile struct {
	Name  string // e.g. "AppDelegate.swift"
	Path  string // e.g. "PhpToro/AppDelegate.swift"
	Group string // e.g. "root", "Bridge", "Engine"
	Type  string // e.g. "sourcecode.swift"
}

func coreSwiftFiles(target string) []pbxFile {
	return []pbxFile{
		{"AppDelegate.swift", target + "/AppDelegate.swift", "root", "sourcecode.swift"},
		{"SceneDelegate.swift", target + "/SceneDelegate.swift", "root", "sourcecode.swift"},
...
type pbxOptions struct {
	...
	TeamID           string
	RuntimeLibDir    string    // absolute path to Libraries/
	PluginFiles      []pbxFile // additional Swift files from plugins
	PluginFrameworks []string  // additional frameworks from plugins
}
...
func generatePbxproj(opts pbxOptions) string {
	...
	// Define fixed UUIDs for structural elements
	...
	if opts.TeamID != "" {
		...
		w(`						DevelopmentTeam = %s;`, opts.TeamID)
		w(`					};`)
		w(`				};`)
	}
	w(`			};`)
	...
	w(`					"$(inherited)",`)
	w(`				);`)
	w(`				IPHONEOS_DEPLOYMENT_TARGET = %s;`, deployTarget)
	...
	if opts.TeamID != "" {
		...
		w(`				DEVELOPMENT_TEAM = %s;`, opts.TeamID)
	}
	w(`			};`)
	w(`			name = Debug;`)
	w(`		};`)
